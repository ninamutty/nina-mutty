...
const Blob = ({ color }: { color: string }): JSX.Element => {
  const NUM_POINTS = 32
  ...
  useEffect(() => {
    ...
    if (canvas && canvas.getContext && center) {
      ...
      const resize = function () {
        ...
        if (center.x !== canvas.width / 2 || center.y !== canvas.height / 2) {
          setCenter({
            x: canvas.width / 2,
            y: canvas.height / 2,
          })
      ...
      const createPoints = () => {
        for (let i = 0; i < NUM_POINTS; i++) {
          let point = new Point(DIVISIONAL * (i + 1), center, radius)
          points.push(point)
        }
      }
      ...
      const mouseMove = (e: MouseEvent) => {
        ...
        if (Math.abs(angle) > 0) {
          ...
          if (nearestPoint) {
            ...
            nearestPoint.acceleration = (strengthValue / 100) * (hover ? -1 : 1)
          }
        }
      ...
      const drawBlob = () => {
        ...
        pointsArray[0].solveWith(pointsArray[NUM_POINTS - 1], pointsArray[1])
    ...
    <canvas className="absolute" id="blob" width={size} height={size}>
      <div
        className={classNames(
          ...
          "dark:bg-gray-100",
          useWidth
            ? "lg:w-blobWidthLg lg:h-blobWidthLg"
            : "lg:w-blobHeightLg lg:h-blobHeightLg",
          useWidth
            ? "md:w-blobWidthMd md:h-blobWidthMd"
            : "md:w-blobHeightMd md:h-blobHeightMd",
          useWidth
            ? "w-blobWidthSm h-blobWidthSm"
            : "w-blobHeightSm h-blobHeightSm"
        )}
      />
    </canvas>
  )
